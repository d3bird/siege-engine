...
private:
	//helper functions
	void offset_world_map(map_data*** input);//moves the objects to their respective spots
	void offset_world_map(map_data* input);//moves the objects to their respective spots

	void create_motion_systems(map_data*** input);// creates the motion systems for the maps
	void create_motion_systems(map_data* input);

	void create_thermal_systems(map_data*** input);// creates the thernak systems for the maps
	void create_thermal_systems(map_data* input);

	bool check(map_data*** input);// checks the generated maps for errors
	bool check(map_data* input);

	//creates the que and returns the index point for that que
	int create_optimised_rendering_que(map_data* input, bool set_primary = false);
