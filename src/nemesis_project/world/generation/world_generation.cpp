...
map_data* world_generator::create_blank_world_cell(int x_siz, int y_siz, int z_siz, bool spawn_bottom) {
...
map_data*** world_generator::create_full_world_map(world_gen_settings* settings) {
	...
	output = new map_data * *[settings->cells_wide];
	for (int i = 0; i < settings->cells_wide; i++) {
		output[i] = new map_data * [settings->cells_long];
	}
...
void world_generator::generate_test_world(map_data* input) {
...
void world_generator::generate_valley_world(map_data* input) {
	int depth = 3;
	int width = 5;
...
void world_generator::generate_hill_world(map_data* input) {
...
void world_generator::generate_forest_world(map_data* input) {
	if (input == NULL) {
		return;
	}
...
void world_generator::generate_room_test(map_data* input) {
	place_square(0, 0, 0, 5, 5, input);
	place_square(7, 0, 7, 4, 4, input);

}

void world_generator::generate_hill(int x, int y, int z, int radius, int height, map_data* input) {
...
void world_generator::place_square(int x, int y, int z, int x_width, int z_width, map_data* input) {
...
void world_generator::place_circle(int x0, int layer, int y0, int radius, bool fill, bool add_floor, map_data* input) {
...
void world_generator::generate_plane_scroll(map_data* input, bool spawn_items) {
	if (input == NULL) {
	...
	int y = 0;
		for (int x = 0; x < x_size; x++) {
			for (int z = 0; z < z_size; z++) {
				...
				if (is_inbounds_of_map(x, y, z, input)) {
...
void world_generator::generate_hill_scroll(map_data* input, bool spawn_items) {

}

//fills in the cliffs with ramps
void world_generator::smooth_in_ramps(map_data* input) {
				...
				if (is_inbounds_of_map(x, y, z, input)) {
					if (world_map[y][x][z].ground != NULL) {
						if (world_map[y][x][z].ground->type == WALL_TEMP) {
							if (is_inbounds_of_map(x - 1, y, z, input)) {
							...
							if (is_inbounds_of_map(x, y, z - 1, input)) {
							...
							if (is_inbounds_of_map(x - 1, y, z - 1, input)) {
...
void world_generator::add_in_missing_floors(map_data* input) {
	int missing = 0;
	map_cell*** world_map = input->map;
	for (int y = 0; y < input->y_size; y++) {
		for (int x = 0; x < input->x_size; x++) {
...
void world_generator::place_furnaces(int x, int y, int z, int x_width, int z_width, map_data* input) {
	...
	if (is_inbounds_of_map(x, y, z + 2, input)) {
...
void world_generator::print_layer(int layer, int print_type, map_data* input) {
	int y = layer;
...
void world_generator::generate_air_drop(map_data* input) {
	if (input == NULL) {
		return;
	}
...
//this function will take the main map_data and create ramps to match the height of the sournding
//cells if they have a higher ground level
void world_generator::smooth_cells(map_data* main, map_data* left, map_data* right,
	map_data* north, map_data* south) {
