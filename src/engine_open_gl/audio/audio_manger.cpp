...
audio_manger::~audio_manger() {
	...
	for (size_t i = effects.size() -1; i >= 0; i--) {
	...
	for (size_t i = background_music.size() - 1; i >= 0; i--) {
...
void audio_manger::create_sound_data() {
	...
	temp_sound_data->sound_data = engine->addSoundSourceFromFile(paths.get_audio_loc("effects/explosion.wav").c_str());
	...
	temp_sound_data = new sound;
	temp_sound_data->sound_data = engine->addSoundSourceFromFile(paths.get_audio_loc("unused_audio/Explosion_Large_Blast_1.mp3").c_str());
	if (temp_sound_data->sound_data == 0) {
	...
	temp_sound_data = new sound;
	temp_sound_data->sound_data = engine->addSoundSourceFromFile(paths.get_audio_loc("unused_audio/Explosion_Large_Blast_2.mp3").c_str());
	if (temp_sound_data->sound_data == 0) {
	...
	temp_sound_data = new sound;
	temp_sound_data->sound_data = engine->addSoundSourceFromFile(paths.get_audio_loc("effects/chicken-alarm-call.wav").c_str());
	if (temp_sound_data->sound_data == 0) {
		std::cout << "there was a problem importing sound 2" << std::endl;
	}
	else {
	...
	temp_sound_data = new sound;
	temp_sound_data->sound_data = engine->addSoundSourceFromFile(paths.get_audio_loc("background_music/space.ogg").c_str());
	if (temp_sound_data->sound_data == 0) {
